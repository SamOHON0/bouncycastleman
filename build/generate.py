...
.mast-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.02fr);
  gap:clamp(24px,3vw,52px);align-items:stretch;position:relative;z-index:2}
.mast-left{display:flex;flex-direction:column;justify-content:center;
  gap:clamp(24px,2.8vw,38px);min-width:0}
...
.mast-shot{position:relative;display:flex;flex-direction:column;min-width:0}
.shot-frame{position:relative;flex:1;display:flex;min-height:0}
.shot-frame .pic{flex:1}
...
.mast-shot .pic{border:3px solid var(--ink);box-shadow:12px 12px 0 var(--c-castle);
  background:var(--paper-2);aspect-ratio:auto;height:100%;min-height:440px}
...
@media(max-width:980px){
  ...
  .mast-left{gap:26px}
  .dpick{max-width:360px}
  .mast-shot{max-width:640px}
  .mast-shot .pic{height:auto;min-height:0;aspect-ratio:4/3}}
...
/* 35% not 50%: the frame is tall and the photos are landscape, so a centred
   crop spends its bottom third on grass and tarmac. */
.slides .slide{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;
  object-position:center 35%;opacity:0;transition:opacity .7s ease}
